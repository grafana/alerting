...
// PostableMimirReceiverToPostableGrafanaReceiver converts legacy Mimir integrations from legacy into
// a PostableApiReceiver with equivalent Grafana-native integrations.
func PostableMimirReceiverToPostableGrafanaReceiver(legacy compat.Receiver) (*definition.PostableApiReceiver, error) {
	v0, err := ConfigReceiverToMimirIntegrations(legacy)
	if err != nil {
	...
	result := &definition.PostableApiReceiver{
		...
		PostableGrafanaReceivers: definition.PostableGrafanaReceivers{
			GrafanaManagedReceivers: make([]*definition.PostableGrafanaReceiver, 0, len(v0)),
		},
	}
	...
	for _, config := range v0 {
		...
		integration, err := MimirIntegrationConfigToPostableGrafanaReceiver(config, legacy.Name, idx)
		if err != nil {
...
// ConfigReceiverToMimirIntegrations converts a compat.Receiver to a list of MimirIntegrationConfig
func ConfigReceiverToMimirIntegrations(receiver compat.Receiver) ([]MimirIntegrationConfig, error) {
