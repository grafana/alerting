...
func NewGrafanaAlertmanager(opts GrafanaAlertmanagerOpts) (*GrafanaAlertmanager, error) {
	...
	am.templates, err = templates.NewCachedFactory(nil, am.logger, am.ExternalURL(), fmt.Sprintf("%d", am.TenantID()))
	if err != nil {
		return nil, err
	}
...
func (am *GrafanaAlertmanager) ApplyConfig(cfg NotificationsConfiguration) (err error) {
	cache, err := templates.NewCachedFactory(cfg.Templates, am.logger, am.ExternalURL(), fmt.Sprintf("%d", am.TenantID()))
	if err != nil {
		return err
	}
	am.templates = cache
