...
var NoWrap WrapNotifierFunc = func(cfg NotifierConfigBase, notifier notify.Notifier) notify.Notifier { return notifier }
...
func BuildGrafanaReceiverIntegrations(
	...
	var (
		...
		errs         error
		ci           = func(idx int, cfg NotifierConfigBase, newInt func(cli *http.Client) notificationChannel) {
			client, err := http.NewClient(cfg.HTTPClientConfig, httpClientOptions...)
			if err != nil {
			...
			i := NewIntegration(wrapped, n, cfg.Type, idx, cfg.Name, notificationHistorian)
	...
	for i, cfg := range receiver.AlertmanagerConfigs {
		ci(i, cfg.NotifierConfigBase, func(_ *http.Client) notificationChannel {
			return alertmanager.New(cfg.Settings, cfg.Metadata, img, logger)
		})
	}
	for i, cfg := range receiver.DingdingConfigs {
		ci(i, cfg.NotifierConfigBase, func(cli *http.Client) notificationChannel {
