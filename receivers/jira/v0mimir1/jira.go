...
// convertToMarshalMap recursively converts maps with non-string keys to maps with string keys
// for JSON marshaling. This is needed because JIRA API requires all map keys to be strings.
func convertToMarshalMap(v any) (any, error) {
	switch val := v.(type) {
	case map[string]any:
		result := make(map[string]any, len(val))
		for k, v := range val {
			converted, err := convertToMarshalMap(v)
			if err != nil {
				return nil, err
			}
			result[k] = converted
...
func (n *Notifier) prepareIssueRequestBody(ctx context.Context, logger log.Logger, groupID string, tmplTextFunc templateFunc) (issue, error) {
	...
	fieldsWithStringKeys, err := convertToMarshalMap(n.conf.Fields)
	if err != nil {
		return issue{}, fmt.Errorf("convertToMarshalMap: %w", err)
	}

	// Convert to map[string]any if not already
	var fields map[string]any
	if fieldsWithStringKeys != nil {
		var ok bool
		fields, ok = fieldsWithStringKeys.(map[string]any)
		if !ok {
			fields = nil
		}
	}
