...
// Notifier is responsible for sending alert notifications to Jira Service Management.
...
func (on *Notifier) Notify(ctx context.Context, as ...*types.Alert) (bool, error) {
	on.log.Debug("executing Jira Service Management notification", "notification", on.Name)

	alerts := types.Alerts(as...)
	if alerts.Status() == model.AlertResolved && !on.SendResolved() {
		on.log.Debug("not sending a trigger to Jira Service Management", "status", alerts.Status(), "auto resolve", on.SendResolved())
		return true, nil
	}

	body, url, err := on.buildJsmMessage(ctx, alerts, as)
	if err != nil {
		return false, fmt.Errorf("build Jira Service Management message: %w", err)
	}
	...
	if err := on.ns.SendWebhook(ctx, cmd); err != nil {
		return false, fmt.Errorf("send notification to Jira Service Management: %w", err)
	}

	return true, nil
}

func (on *Notifier) buildJsmMessage(ctx context.Context, alerts model.Alerts, as []*types.Alert) (payload []byte, apiURL string, err error) {
	...
	message, truncated := receivers.TruncateInRunes(tmpl(on.settings.Message), jsmMaxMessageLenRunes)
	if truncated {
		on.log.Warn("Truncated message", "alert", key, "max_runes", jsmMaxMessageLenRunes)
	}
	...
	if tmplErr != nil {
		on.log.Warn("failed to template Jira Service Management message", "error", tmplErr.Error())
		tmplErr = nil
	}
	...
	if tmplErr != nil {
		on.log.Warn("failed to template Jira Service Management URL", "error", tmplErr.Error(), "fallback", on.settings.APIUrl)
