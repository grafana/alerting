...
func (r *PostableApiReceiver) UnmarshalYAML(unmarshal func(interface{}) error) error {
	...
	if hasGrafanaReceivers {
		if len(r.DiscordConfigs) > 0 {
			return fmt.Errorf("cannot have both Alertmanager DiscordConfigs & Grafana receivers together")
		}
		...
		if len(r.MSTeamsConfigs) > 0 {
			return fmt.Errorf("cannot have both Alertmanager MSTeamsConfigs & Grafana receivers together")
		}
		...
		if len(r.SNSConfigs) > 0 {
			return fmt.Errorf("cannot have both Alertmanager SNSConfigs & Grafana receivers together")
		}
		if len(r.TelegramConfigs) > 0 {
			return fmt.Errorf("cannot have both Alertmanager TelegramConfigs & Grafana receivers together")
		}
		...
		if len(r.WebexConfigs) > 0 {
			return fmt.Errorf("cannot have both Alertmanager WebexConfigs & Grafana receivers together")
		}
